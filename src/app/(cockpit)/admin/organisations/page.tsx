...
export default function AdminOrganisationsPage() {
  ...
  return (
    <div style={{ maxWidth: 1000, fontFamily: "var(--font-jakarta)" }}>
      <p style={{ fontSize: 14, color: "#8A8A96", marginBottom: 24 }}>
      ...
      {!loading && orgs.length === 0 ? (
        <div className="admin-card-static" style={{ textAlign: "center", padding: "48px 24px" }}>
          ...
          <p style={{ fontSize: 14, fontWeight: 600, color: "#8A8A96" }}>Aucune organisation</p>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {orgs.map((org, i) => (
            ...
            >
              ...
              {org.type && (
                <p style={{ fontSize: 12, color: "#8A8A96", marginBottom: 10 }}>{org.type}</p>
              )}
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 4 }}>
                <Users size={12} style={{ color: "#8A8A96" }} />
                <span style={{ fontSize: 12, color: "#8A8A96" }}>
                  {org._count?.members ?? "—"} membre{(org._count?.members ?? 0) > 1 ? "s" : ""}
                </span>
              </div>
              <p style={{ fontSize: 11, color: "#8A8A96", marginTop: 8 }}>
