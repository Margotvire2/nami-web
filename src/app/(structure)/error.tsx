 ...
 *
 * Error boundary du groupe (structure) — espace orga / réseau de soins.
 * - Sentry tag `boundary=structure`
...
export default function StructureError({
  ...
  useEffect(() => {
    Sentry.withScope((scope) => {
      scope.setTag("boundary", "structure");
      scope.setTag("error.digest", error.digest ?? "unknown");
      Sentry.captureException(error);
    });
    if (process.env.NODE_ENV !== "production") {
      console.error("[Nami] Structure error:", error);
    }
  }, [error]);
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: "#FAFAF8" }}>
      ...
      >
        ...
        <p className="text-sm mb-4 leading-relaxed" style={{ color: "#6B7280" }}>
          Vous pouvez réessayer ou revenir à l&apos;espace structure. Notre équipe a été notifiée.
          {error.digest && (
