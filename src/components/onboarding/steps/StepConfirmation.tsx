...
const DELAY_LABELS: Record<string, string> = {
  WITHIN_WEEK:  "Sous 1 semaine",
  WITHIN_MONTH: "Sous 1 mois",
  OVER_MONTH:   "Plus d'1 mois",
}
...
export function StepConfirmation() {
  ...
  // [LEGAL] Checkboxes CGU obligatoires avant soumission
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="space-y-2">
      ...
      <div className="p-4 rounded-xl bg-neutral-50 border border-neutral-100 text-sm text-neutral-700 space-y-1">
        {children}
      </div>
    </div>
  )
  ...
  const Tag = ({ label }: { label: string }) => (
    <span className="inline-block px-2 py-0.5 rounded-full bg-teal-100 text-teal-700 text-xs mr-1.5 mb-1.5">
      {label}
    </span>
  )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-1">
          Vérifiez vos informations avant de valider votre profil.
        </p>
      </div>
      ...
      <div className="space-y-4 max-h-[45vh] overflow-y-auto pr-1">
        ...
        <Section title="Identité professionnelle">
          {data.specialties?.length ? (
            <div>
              {data.specialties.map((id) => (
                <Tag key={id} label={getSpecialtyLabel(id)} />
              ))}
            </div>
          ) : null}
          {data.subSpecialties?.length ? (
            <div>
          ...
          {data.bio && (
            <p className="text-xs text-neutral-500 mt-2 line-clamp-3">{data.bio}</p>
          )}
        </Section>
        ...
        <Section title="Mode d'exercice">
          {data.exerciseMode && (
            <div>
              <Tag label={MODE_LABELS[data.exerciseMode] || data.exerciseMode} />
            </div>
          )}
          ...
          <div className="flex gap-4 pt-1 text-xs text-neutral-500">
            {data.acceptsCMU && <span>CMU-C</span>}
            {data.acceptsALD && <span>ALD</span>}
            {data.acceptsTele && <span>Téléconsultation</span>}
          </div>
        </Section>
          ...
          {data.consultationModes?.length ? (
            <div>
              {data.consultationModes.map((m) => (
                <Tag key={m} label={CONSULTATION_MODE_LABELS[m] || m} />
              ))}
            </div>
          ) : null}
          {data.acceptedPatientTypes?.length ? (
            <div>
              {data.acceptedPatientTypes.map((t) => (
                <Tag key={t} label={t} />
              ))}
            </div>
          ) : null}
          ...
          {data.certifications?.length ? (
            <div className="space-y-1">
              {data.certifications.map((c, i) => (
                <p key={i} className="text-xs">
                  {c.name} {c.organism && `— ${c.organism}`} {c.year && `(${c.year})`}
                </p>
              ))}
            </div>
          ) : (
            <p className="text-neutral-400 italic">Aucune certification</p>
          )}
        </Section>
        ...
        <Section title="Réseau & Visibilité">
          ...
          {data.geographicZones?.length ? (
            <div>
              {data.geographicZones.map((z) => (
                <Tag key={z} label={z} />
              ))}
            </div>
          ) : null}
      ...
      <div className="space-y-3 pt-2">
        ...
        {/* [LEGAL] Déontologie — attestation absence de sanction disciplinaire */}
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            type="checkbox"
            checked={acceptedDeonto}
            onChange={(e) => setAcceptedDeonto(e.target.checked)}
            className="mt-0.5 h-4 w-4 rounded border-neutral-300 text-teal-600 focus:ring-teal-500"
          />
          <span className="text-sm text-neutral-600">
            J&apos;atteste que les informations renseignées sont exactes, que je suis
            inscrit(e) à l&apos;Ordre compétent et qu&apos;aucune sanction disciplinaire
            en cours ne m&apos;interdit d&apos;exercer.
          </span>
        </label>

        {/* [LEGAL] RGPD Art. 13 — information obligatoire */}
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            type="checkbox"
          ...
          <span className="text-sm text-neutral-600">
            ...
            et j&apos;accepte que mes données professionnelles soient traitées
