...
const CONSULTATION_MODES = [
  {
    id:          "IN_PERSON",
    label:       "Présentiel",
    description: "Consultation au cabinet ou en structure",
  },
  {
    id:          "TELECONSULTATION",
    label:       "Téléconsultation",
    // [LEGAL] Décret 2018-788 + avenant 6 convention médicale
    description: "Vidéo uniquement — cadre HAS obligatoire",
  },
  {
    id:          "HOME_VISIT",
    label:       "Visite à domicile",
    description: "Déplacement chez le patient",
  },
  {
    id:          "HYBRID",
    label:       "Hybride",
    description: "Combinaison selon les situations",
  },
]

const PATIENT_TYPES = [
  { id: "ADULT",      label: "Adultes" },
  { id: "CHILD",      label: "Enfants" },
  { id: "ADOLESCENT", label: "Adolescents" },
  { id: "ELDERLY",    label: "Personnes âgées" },
  { id: "PERINATAL",  label: "Périnatal / Périnatalité" },
...
export function StepConsultation() {
  ...
  const validate = () => {
    ...
    if (!data.consultationModes?.length) {
      e.consultationModes = "Sélectionnez au moins un mode"
    }
    if (!data.acceptedPatientTypes?.length) {
      e.acceptedPatientTypes = "Sélectionnez au moins un type de patient"
    }
    setErrors(e)
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-1">
          Ces informations guident les adressages des confrères
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        {/* [LEGAL] Avertissement téléconsultation */}
        {hasTeleconsult && (
          <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
            <Info className="w-4 h-4 mt-0.5 flex-shrink-0" />
            <div>
              <p className="font-medium">Téléconsultation — cadre réglementaire</p>
              <p className="mt-0.5">
                La téléconsultation doit respecter le Décret n°2018-788 et
                l&apos;avenant 6 à la convention médicale. Le consentement du patient
                est obligatoire. Un compte-rendu doit être transmis au médecin traitant.
              </p>
            </div>
          </div>
        )}
        ...
        <label className="flex items-center justify-between p-4 rounded-xl border border-neutral-200 cursor-pointer hover:bg-neutral-50">
          <div>
            ...
            <p className="text-xs text-neutral-400 mt-0.5">
              Visible sur votre fiche et pour les adressages
            </p>
          </div>
          <input
            type="checkbox"
