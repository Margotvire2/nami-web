    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Section icon={<CheckCircle size={14} color="#2BA89C" />} title={`Conformité — ${compliance.percentage}%`} accent="#2BA89C">
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: "#374151" }}>{compliance.score}/{compliance.total}</span>
        </div>
          ...
          {ctx.bloc3_nextSteps.map((step, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, paddingBottom: 8, borderBottom: i < ctx.bloc3_nextSteps.length - 1 ? "1px solid rgba(26,26,46,0.06)" : "none" }}>
              <div style={{ flex: 1 }}>
                ...
                {step.specialty && <p style={{ fontSize: 11, color: "#6B7280", margin: "2px 0 0" }}>{step.specialty}{step.phase ? ` · ${step.phase}` : ""}</p>}
              </div>
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                {step.expectedDate && <p style={{ fontSize: 11, color: "#6B7280", margin: 0 }}>{step.expectedDate}</p>}
        ...
        <Section icon={<AlertTriangle size={14} color="#D94F4F" />} title={`Alertes (${ctx.bloc4_alerts.length})`} accent="#D94F4F">
          {ctx.bloc4_alerts.map((a, i) => (
            <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: SEVERITY_COLOR[a.severity] ?? "#6B7280", marginTop: 5, flexShrink: 0 }} />
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, color: "#2D2B3D", margin: 0 }}>{a.title}</p>
                {a.description && <p style={{ fontSize: 12, color: "#6B7280", margin: "2px 0 0" }}>{a.description.slice(0, 120)}</p>}
              </div>
            </div>
          ))}
        </Section>
          ...
          {ctx.bloc5_questionnaires.map((q, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: "#6B7280" }}>({q.date})</span>
              </div>
            </div>
          ))}
        </Section>
          ...
          {ctx.bloc6_teamSynthesis.map((m, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              ...
              <div>
                ...
                {m.specialties.length > 0 && <p style={{ fontSize: 11, color: "#6B7280", margin: "1px 0 0" }}>{m.specialties.join(", ")}</p>}
              </div>
            </div>
          ))}
        </Section>
